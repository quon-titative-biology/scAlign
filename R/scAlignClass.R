...
scAlignCreateObject = function(sce.objects,
  ...
  if(length(sce.objects)==2){
    ...
    combined.sce@colData@listData[["group.by"]] = c(rep(names(sce.objects)[1], ncol(sce.objects[[names(sce.objects)[1]]])),
                                                    rep(names(sce.objects)[2], ncol(sce.objects[[names(sce.objects)[2]]])))
    ## Set labels to be used in supervised training or defaults
    combined.sce@colData@listData[["scAlign.labels"]] = if(length(unlist(labels)) == 0) rep("NA", combined.sce@colData@nrows) else unlist(labels)
    if(length(combined.sce@colData@listData[["scAlign.labels"]]) < combined.sce@colData@nrows) { stop("Not enough labels for all cells.") }
    combined.sce@metadata[["arguments"]] = data.frame(encoder.data=character(),
    ...
    combined.sce@metadata[["options"]]  = data.frame(steps=integer(),
                                            batch.size=integer(),
  ...
  if(is.element(data.use, names(combined.sce@assays)) == FALSE){ stop("data.use is not reachable in object@assays.") }

  ## Reduce to top 50 pcs
  if(pca.reduce == TRUE){
    print(paste("Computing partial PCA for top ", pcs.compute, " PCs."))
    pca_results = irlba(A = t(combined.sce@assays[[data.use]]), nv = pcs.compute)
    reducedDim(combined.sce, "PCA") = pca_results$u
    combined.sce@metadata[["PCA.output"]] = pca_results
  }
  ...
  if(cca.reduce == TRUE){
    ...
    for(name in names(sce.objects)){
      sce.objects[[name]] = CreateSeuratObject(raw.data = sce.objects[[name]]@assays[[data.use]], project = name, min.cells = 0)
    ...
    combined = RunCCA(sce.objects[[names(sce.objects)[1]]],
                      ...
                      genes.use=combined.sce@rowRanges@partitioning@NAMES,
                      num.cc=ccs.compute,
                      scale.data=TRUE)
    ## Load dim reduction into SCE object
    reducedDim(combined.sce, "CCA") = combined@dr$cca@cell.embeddings
    combined.sce@metadata[["CCA.output"]] = combined@dr$cca@gene.loadings
  }
  return(combined.sce)
}
