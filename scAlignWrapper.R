...
#' 
#' @export
as.SingleCellExperimentList = function(seurat.obj, genes.use){
  if(is.null(genes.use)){
    genes.use = as.numeric(nrow(seurat.obj))
  }else{
  sce.objects = lapply(seurat.obj, function(seurat.obj){
    SingleCellExperiment(assays = list(counts = (GetAssayData(object = seurat.obj, slot = "counts"))[genes.use,],
                                       logcounts = (GetAssayData(object = seurat.obj, slot = "data"))[genes.use,],
                                       scale.data = (GetAssayData(object = seurat.obj, slot = "scale.data"))[genes.use,]),
                                       colData = seurat.obj[[]])})
  return(sce.objects)
  }
}
