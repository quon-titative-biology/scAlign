...
encoderModel_gaussian_kernel = function(data, data_b=NULL, dim, perplexity=30, method="euclidean", betas.fixed=0, diag="zero"){
  with(tf$compat$v1$name_scope("gaussian_kernel"), {
    ...
    ## distances
    if(method == "euclidean"){
      norm_a = tf$compat$v1$reshape(tf$compat$v1$reduce_sum(tf$compat$v1$square(data), as.integer(1)), c(as.integer(-1), as.integer(1)))
      norm_b = tf$compat$v1$reshape(tf$compat$v1$reduce_sum(tf$compat$v1$square(data_b), as.integer(1)), c(as.integer(1), as.integer(-1)))
    ...
    # }else{
      ...
      body = function(step, affinities, perplexity, betas, beta_min, beta_max, p_ij, p_ij_unnorm){
        ...
        p_ij = tf$compat$v1$exp(tf$compat$v1$multiply(tf$compat$v1$negative(affinities), tf$compat$v1$reshape(betas, c(as.integer(-1), as.integer(1)))))
        p_ij = tf$compat$v1$reshape(p_ij, shape(dim, dim)) ## tensorflow R requires this?
        ## Reset exp(0)'s to 0
        if(diag == "zero"){
          p_ij = tf$compat$v1$matrix_set_diag(p_ij, tf$compat$v1$cast(tf$compat$v1$zeros(shape(dim)), tf$compat$v1$float64))
        ...
        p_ij_unnorm = p_ij
        ## Normalize
        p_ij = tf$compat$v1$divide(p_ij, tf$compat$v1$reshape(sum_pij, c(as.integer(-1), as.integer(1))))
