...
## Data setup
The gene count matrices for this tutorial are hosted [here](https://github.com/quon-titative-biology/data/blob/master/kowalcyzk_gene_counts.rda).
...
youngMouseSCE <- SingleCellExperiment(
    assays = list(counts = youngMouseSeuratObj@raw.data[genes.use,], scale.data = youngMouseSeuratObj@scale.data[genes.use,])
)

oldMouseSCE <- SingleCellExperiment(
    assays = list(counts = oldMouseSeuratObj@raw.data[genes.use,], scale.data = oldMouseSeuratObj@scale.data[genes.use,])
)
...
scAlignHSC = scAlign(scAlignHSC,
                     options=scAlignOptions(steps=5000, norm=TRUE, log.every=3000),
...
scAlignHSC = scAlign(scAlignHSC,
                     options=scAlignOptions(steps=15000, norm=TRUE, log.every=3000),
                     encoder.data="PCA",
                     supervised='none',
                     run.encoder=TRUE,
                     run.decoder=FALSE,
